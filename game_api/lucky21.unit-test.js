...
describe('Game API', () => {
  ...
  describe('The function convertCard should convert the players cards to numbers', () => {
    ...
    var expectedPlayerCardsNumbers = [10, 9];
  ...
  describe('The function getTotal should get total value should return the total value of the players card', () => {
    ...
    test('getTotal value should return the players cards total value', () => {
      ...
      var playerCardsNumbers = game.state.playerCardsNumbers;
      var currentplayerCards =  game.state.playerCardsNumber;
      game.state.playerCardsNumbers = [10, 9, 10, 5, 3];
      game.getTotal(game);
      expect(game.state.total).toEqual(37);
      game.state.playerCardsNumbers = playerCardsNumbers;
      //Leave the state as it was
      game.state.playerCardsNumber = currentplayerCards;
      game.state.total = 0;
    });
  ...
  describe('The function guessAbove21 should draw the final card', () => {
    ...
    describe('The function guess21OrUnder should draw the next card', () => {
      ...
      describe("The game should continue if there is no win or no gameover", () => {
        ...
        var oldLength = game.state.cards.length; //Initial length
      ...
      describe("guess21OrUnder should produce a game over if total above 21", () => {
        //New playerCardNumbers as it's rather hard to test it if it's always changing
        let game = lucky21Constructor(deck, dealer);//Preventing messing up by using a different game instance
        var playerCardsNumbers = game.state.cards;
        game.state.playerCardsNumbers = [10, 8, 10];
      
        game.guess21OrUnder(game);
       
        test('isGameOver should be true if total above 21', () => {
